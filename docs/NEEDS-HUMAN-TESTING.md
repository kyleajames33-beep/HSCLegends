...
6. **Apply `supabase/migrations/20260924_streak_freeze_repair.sql`** to HSCScienceSyd (not applied by Claude). It replaces `record_daily_quiz` + `record_quick_game` (bodies copied from live on 2026-09-24, only the streak block changed) and adds `get_streak_overview` / `repair_streak`. Until it is applied, `/streak` and the home streak card show nothing. Then test: miss a weekday twice in one week with a bought freeze (should be spent), break a 2+ day streak and repair it with 20 answers.
